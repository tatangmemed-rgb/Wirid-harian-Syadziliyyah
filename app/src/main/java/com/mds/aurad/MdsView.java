...
public class MdsView extends View {
    ...
    protected void onDraw(Canvas c) {
        ...
        if (page == 11) {
            if (latin) drawFull(c, page11Latin);
    ...
    private void drawPage(Canvas c, String title, String first, String key, int target,
        ...
        if (!first.isEmpty()) {
            top = drawParagraph(c, first, w * .08f, w * .92f, top, latin ? 40f : 46f);
            if (key != null) top = drawCounter(c, key, target, top + 16f);
        }

        if (second != null) {
            top = drawParagraph(c, second, w * .08f, w * .92f, top + 32f, latin ? 38f : 44f);
            if (key2 != null) drawCounter(c, key2, target2, top + 20f);
        }
    ...
    private void drawFooter(Canvas c) {
        float w = getWidth(), h = getHeight();
        button(c, new RectF(w * .10f, h * .83f, w * .90f, h * .89f),
                latin ? "GANTI KE TEKS ARAB" : "GANTI KE TEKS LATIN", false);
        button(c, new RectF(w * .07f, h * .91f, w * .47f, h * .975f),
                "‹  KEMBALI", false);
        button(c, new RectF(w * .53f, h * .91f, w * .93f, h * .975f),
    ...
    private boolean handleCounterTouch(float x, float y, float w, float h) {
        ...
        if (page == 4 && x >= w * .15f && x <= w * .85f) {
            if (y > h * .32f && y < h * .55f) { key = "p4a"; max = 3; }
            else if (y >= h * .55f && y < h * .82f) { key = "p4b"; max = 3; }
        } else if (page == 5 && y > h * .48f && y < h * .82f) {
            key = "p5"; max = 99;
        } else if (page == 7 && y > h * .30f && y < h * .82f) {
            key = "p7"; max = 99;
        } else if (page == 8 && y > h * .30f && y < h * .82f) {
            key = "p8"; max = 99;
        } else if (page == 9 && y > h * .45f && y < h * .82f) {
            key = "p9"; max = 3;
        }
