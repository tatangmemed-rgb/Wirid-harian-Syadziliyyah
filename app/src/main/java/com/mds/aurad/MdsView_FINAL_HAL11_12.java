...
public class MdsView extends View {
    ...
    protected void onDraw(Canvas c) {
        ...
        // HALAMAN 10: gambar dipotong sedikit di bawah agar nomor 10/22 hilang,
        // lalu memakai tombol aplikasi yang aktif.
        if (page == 10) {
            drawImagePage(c, latin ? page10Latin : page10Arab, .88f);
            return;
        }

        // HALAMAN 11: gambar asli SUDAH mempunyai tombol. Jangan menggambar
        // footer baru, karena akan membuat tombol dobel. Yang dibuat aktif hanya
        // area sentuh tombol yang memang sudah ada di gambar asli.
        if (page == 11) {
            drawFull(c, latin ? page11Latin : page11Arab);
            return;
        }
        ...
        if (page == 12) {
            drawImagePage(c, latin ? page12Latin : page12Arab, .79f);
            return;
        }
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (page == 11) {
            // Tombol sudah tercetak di gambar asli halaman 11.
            // Posisi mengikuti proporsi gambar asli, tanpa menambah footer baru.
            if (x >= w * .18f && x <= w * .82f && y >= h * .69f && y <= h * .75f) {
                latin = !latin;
                invalidate();
                return true;
            }
            if (x >= w * .05f && x <= w * .49f && y >= h * .74f && y <= h * .81f) {
                page = 10;
                invalidate();
                return true;
            }
            if (x >= w * .51f && x <= w * .95f && y >= h * .74f && y <= h * .81f) {
                page = 12;
                invalidate();
                return true;
            }
