...
public class MdsView extends View {
    ...
    protected void onDraw(Canvas c) {
        ...
        // Halaman gambar tetap memakai footer sendiri agar tombol GANTI TEKS,
        // KEMBALI dan LANJUT/SELESAI selalu terlihat dan bisa disentuh.
        if (page == 10) {
            drawImagePage(c, latin ? page10Latin : page10Arab, true);
            return;
        }
        if (page == 11) {
            if (latin) drawImagePage(c, page11Latin, false);
            else drawTextPage(c, "التَّوَسُّلُ", arab11(), true);
            return;
        }
        if (page == 12) {
            drawImagePage(c, latin ? page12Latin : page12Arab, false);
            return;
        }
    ...
    // Untuk halaman yang berupa gambar, sisakan bagian bawah untuk tombol.
    // Pada halaman 10, bagian paling bawah gambar dipotong agar nomor "10/22"
    // tidak ikut tampil. Dengan ruang footer yang lebih besar, tombol GANTI
    // TEKS juga dibuat lebih besar dan mudah disentuh.
    private void drawImagePage(Canvas c, Bitmap b, boolean cropBottomNumber) {
        ...
        if (b != null) {
            ...
            if (cropBottomNumber) {
                // Potong 12% bagian bawah gambar sumber yang berisi nomor 10/22.
                srcBottom = Math.max(1, Math.round(b.getHeight() * .88f));
            }
    ...
    private void drawPage(Canvas c, String title, String first, String key, int target,
        ...
        // Huruf Arab halaman 4-9 dibuat lebih besar agar lebih nyaman dibaca.
        float firstSize = latin ? 40f : 58f;
        float secondSize = latin ? 38f : 54f;

        if (!first.isEmpty()) {
            top = drawParagraph(c, first, w * .05f, w * .95f, top, firstSize);
            if (key != null) top = drawCounter(c, key, target, top + 16f);
        }

        if (second != null) {
            top = drawParagraph(c, second, w * .05f, w * .95f, top + 24f, secondSize);
    ...
    private String arab11() {
        return "اللَّهُمَّ إِنِّي أَتَوَسَّلُ إِلَيْكَ بِنَبِيِّكَ مُحَمَّدٍ صَلَّى اللَّهُ عَلَيْهِ وَسَلَّمَ، وَبِآلِهِ وَصَحْبِهِ، وَأَسْأَلُكَ مِنْ فَضْلِكَ وَرَحْمَتِكَ، إِنَّكَ أَنْتَ الْوَهَّابُ.";
    }
}
