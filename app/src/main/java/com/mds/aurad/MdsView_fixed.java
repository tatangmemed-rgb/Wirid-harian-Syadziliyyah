...
public class MdsView extends View {
    ...
    @Override protected void onDraw(Canvas c) {
        ...
        if (page == 10) { drawImagePage(c, latin ? page10Latin : page10Arab, latin ? .88f : 1.00f); return; }
        if (page == 11) { drawImagePage(c, latin ? page11Latin : page11Arab, .90f); return; }
        if (page == 12) { drawImagePage(c, latin ? page12Latin : page12Arab, .90f); return; }
        drawContent(c);
    }

    private void drawFull(Canvas c, Bitmap b) {
        if (b == null) return;
        c.drawBitmap(b, new Rect(0,0,b.getWidth(),b.getHeight()), new RectF(0,0,getWidth(),getHeight()), p);
    }
    ...
    private void drawImagePage(Canvas c, Bitmap b, float sourceBottom) {
        ...
        c.drawColor(BG);
        if (b != null) {
            int srcBottom=Math.max(1,Math.min(b.getHeight(),Math.round(b.getHeight()*sourceBottom)));
            c.drawBitmap(b,new Rect(0,0,b.getWidth(),srcBottom),new RectF(0,0,w,h*.79f),p);
        }
        drawFooter(c);
    }
    ...
    private void header(Canvas c, String title) {
        ...
        if (logo != null) c.drawBitmap(logo,null,new RectF((w-size)/2f,25,(w+size)/2f,25+size),p);
        p.setColor(GOLD);
    ...
    private void drawContent(Canvas c) {
        ...
        if (page==4) {
            ...
            first=latin?"AUDZUBILLAHIMNASSYAITHONIRROJIIM (3X)":"أَعُوذُ بِاللَّهِ مِنَ الشَّيْطَانِ الرَّجِيمِ (٣×)";
            key="p4a"; target=3;
            second=latin?"BISMILLAAHIRROHMAANIRROHIIM (3X)":"بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ (٣×)";
            key2="p4b"; target2=3;
        } else if (page==5) {
            first=latin?"WAMAA TUQODDIMUU LI ANFUSIKUM MIN KHOIRIN TAJIDUUHU 'INDALLOOHI HUWA KHOIRON WA A'ZHOMA AJRON WASTAGHFIRULLOOH. INNALLOOHA GHOFUURUR ROHIIM.":"وَمَا تُقَدِّمُوا لِأَنْفُسِكُمْ مِنْ خَيْرٍ تَجِدُوهُ عِنْدَ اللَّهِ هُوَ خَيْرًا وَأَعْظَمُ أَجْرًا وَاسْتَغْفِرُوا اللَّهَ إِنَّ اللَّهَ غَفُورٌ رَحِيمٌ.";
            second=latin?"ASTAGHFIRULLAAHAL 'AZHIIM (99X)":"أَسْتَغْفِرُ اللَّهَ الْعَظِيمَ (٩٩×)";
            key2="p5"; target2=99;
        } else if (page==6) {
            first=latin?"ASTAGHFIRULLAAHAL 'AZHIIM. ALLADZII LAA ILAAHA ILLAA HUWAL HAYYUL QOYYUUMU WA ATUUBU ILAIH":"أَسْتَغْفِرُ اللَّهَ الْعَظِيمَ الَّذِي لَا إِلٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ وَأَتُوبُ إِلَيْهِ";
            second=latin?"INNALLAAHA WA MALAAIKATAHUU YUSHOLLUUNA 'ALAN NABIYYI. YAA AYYUHALLADZIINA AAMANUU SHOLLUU 'ALAIHI WA SALLIMUU TASLIIMAA.":"إِنَّ اللَّهَ وَمَلَائِكَتَهُ يُصَلُّونَ عَلَى النَّبِيِّ يَا أَيُّهَا الَّذِينَ آمَنُوا صَلُّوا عَلَيْهِ وَسَلِّمُوا تَسْلِيمًا.";
        } else if (page==7) {
            first=latin?"ALLAAHUMMA SHOLLI 'ALAA SAYYIDINAA MUHAMMADIN 'ABDIKA WA ROSUULIKAN NABIYYIL UMMIYYI WA 'ALAA AALIHI WA SHOHBIHI WA SALLIM (99X)":"اللَّهُمَّ صَلِّ عَلَى سَيِّدِنَا مُحَمَّدٍ عَبْدِكَ وَرَسُولِكَ النَّبِيِّ الْأُمِّيِّ وَعَلَى آلِهِ وَصَحْبِهِ وَسَلِّمْ (٩٩×)";
            key="p7"; target=99;
            second=latin?"ALLAAHUMMA SHOLLI 'ALAA SAYYIDINAA MUHAMMADIN 'ABDIKA WA ROSUULIKAN NABIYYIL UMMIYYI WA 'ALAA AALIHI WA SHOHBIHI WA SALLIM TASLIIMAN BIQODRI 'AZHOMATI DZAATIKA FII KULLI WAQTIN WA HIIN.":"اللَّهُمَّ صَلِّ عَلَى سَيِّدِنَا مُحَمَّدٍ عَبْدِكَ وَرَسُولِكَ النَّبِيِّ الْأُمِّيِّ وَعَلَى آلِهِ وَصَحْبِهِ وَسَلِّمْ تَسْلِيمًا بِقَدْرِ عَظَمَةِ ذَاتِكَ فِي كُلِّ وَقْتٍ وَحِينٍ.";
        } else if (page==8) {
            title=latin?"FA'LAM ANNAHU":"فَاعْلَمْ أَنَّهُ";
            first=latin?"LAA ILAAHA ILLALLAAH (99X)":"لَا إِلٰهَ إِلَّا اللَّهُ (٩٩×)";
            key="p8"; target=99;
            second=latin?"LAA ILAAHA ILLALLAAHU MUHAMMADUR ROSUULULLAAHI SHOLLALLAAHU 'ALAIHI WA 'ALAA AALIHI WA SHOHBIHI WA SALLAM.":"لَا إِلٰهَ إِلَّا اللَّهُ مُحَمَّدٌ رَسُولُ اللَّهِ صَلَّى اللَّهُ عَلَيْهِ وَعَلَى آلِهِ وَصَحْبِهِ وَسَلَّمَ.";
        } else if (page==9) {
            first=latin?"BISMILLAAHIRROHMAANIRROHIIM.\nQUL HUWALLOOHU AHAD.\nALLOOHUSH SHOMAD.\nLAM YALID WA LAM YUULAD WA LAM YAKUN LAHUU KUFUWAN AHAD (3X)":"بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ.\nقُلْ هُوَ اللَّهُ أَحَدٌ.\nاللَّهُ الصَّمَدُ.\nلَمْ يَلِدْ وَلَمْ يُولَدْ وَلَمْ يَكُنْ لَهُ كُفُوًا أَحَدٌ (٣×)";
            key="p9"; target=3;
        }
    ...
    private void drawPage(Canvas c,String title,String first,String key,int target,String second,String key2,int target2) {
        float w=getWidth(),h=getHeight();
        header(c,title);
        float left=w*.05f,right=w*.95f,topStart=h*.23f,bottomLimit=h*.785f;
        float firstSize=latin?44f:62f,secondSize=latin?42f:58f;
        float minFirst=latin?36f:50f,minSecond=latin?34f:48f;
        while (true) {
    ...
    private float estimateParagraphHeight(String text,float width,float size,boolean arabic){
        ...
        p.setTextSize(size);
        p.setTypeface(arabic?Typeface.create("serif",Typeface.NORMAL):Typeface.create(Typeface.SANS_SERIF,Typeface.BOLD));
        int lines=0;
    ...
    private float drawParagraphMode(Canvas c,String text,float left,float right,float y,float size,boolean arabic){
    ...
    private List<String> wrap(String text,float width,Paint paint){
        ...
        String current="";
        // Hanya halaman 4 dan 9 Latin yang mendapat pemecahan kata panjang.
        boolean splitLongWords=latin&&(page==4||page==9);
        for(String word:text.trim().split(" ")){
            if(splitLongWords&&paint.measureText(word)>width){
                if(!current.isEmpty()){out.add(current);current="";}
                String part="";
                for(int i=0;i<word.length();i++){
                    String nextPart=part+word.charAt(i);
                    if(paint.measureText(nextPart)>width&&!part.isEmpty()){
                        out.add(part);
                        part=String.valueOf(word.charAt(i));
                    }else{
                        part=nextPart;
                    }
                }
    ...
    private float drawCounter(Canvas c,String key,int target,float y){
        ...
        c.drawText("RESET",reset.centerX(),reset.centerY()+p.getTextSize()*.35f,p);
        return reset.bottom+w*.035f;
    }

    private void drawFooter(Canvas c){
        float w=getWidth(),h=getHeight();
        button(c,new RectF(w*.07f,h*.80f,w*.93f,h*.885f),latin?"GANTI KE TEKS ARAB":"GANTI KE TEKS LATIN",false);
        button(c,new RectF(w*.07f,h*.905f,w*.47f,h*.985f),"‹  KEMBALI",false);
        button(c,new RectF(w*.53f,h*.905f,w*.93f,h*.985f),page==12?"SELESAI  ✓":"LANJUT  ›",true);
    }
    ...
    @Override public boolean onTouchEvent(MotionEvent e){
        ...
        if(page==2){page=3;invalidate();return true;}
        if(page==3){
            if(y>=h*.42f&&y<=h*.56f){latin=false;page=4;}
            else if(y>=h*.60f&&y<=h*.74f){latin=true;page=4;}
            invalidate();return true;
        }
        if(page>=4&&page<=12){
            if(x>=w*.07f&&x<=w*.93f&&y>=h*.80f&&y<=h*.885f){latin=!latin;invalidate();return true;}
            if(y>=h*.905f&&x>=w*.07f&&x<=w*.47f){page=Math.max(3,page-1);invalidate();return true;}
            if(y>=h*.905f&&x>=w*.53f&&x<=w*.93f){
                if(page==12){if(getContext() instanceof Activity)((Activity)getContext()).finish();}
                else{page++;invalidate();}
                return true;
            }
            if(handleCounterTouch(x,y,w,h)){invalidate();return true;}
        }
        return true;
    }
